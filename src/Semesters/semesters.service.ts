...
export class SemestersService extends BaseService<Semester> {
    constructor (@InjectModel('Semester') private semesterModel: Model<SemesterDocument>) {
        super(semesterModel);
    }
    ...
    private async getSemesterRespone(semester: Semester): Promise<any> {
        ...
        const {_id, createdAt, updatedAt, ...result} = JSON.parse(JSON.stringify(semester));
        return {
            id: _id,
            ...result,
            week: week
    ...
    private getFirstDayofWeek(date: Date): Date {
        ...
        return new Date(date.getFullYear(), date.getMonth(), date.getDate() - dayBefore[dayDate]);
    }
}
