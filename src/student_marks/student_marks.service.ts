...
export class StudentMarksService extends BaseService<StudentMark> {
    ...
    async findByStudentId(semester: string, studentId: string, subject: string): Promise<StudentMark> {
        return await this.studentMarkModel.findOne({ semester: semester, student: studentId, subject: subject });
    }

    async findAllByStudentId(semester: string, studentId: string): Promise<StudentMark[]> {
        return await this.studentMarkModel.find({ semester: semester, student: studentId });
    }
